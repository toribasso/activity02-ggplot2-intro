...
**Response**: The Gentoo penguins look to be significantly different from the other two penguins.
It has a much larger flipper length. The Chinstrap and Adilie penguins are similar, with Chinstrap tends
to have a slightly longer flipper length and Adelie has long and short flipper outliers.
...
**Response**: You can assess individual outliers better in the jitterplot. In the boxplot, it is less
overwhelming and you can look at the overall distribution. The boxplot seems to be more of a summary.
...
**Response**: I think the Rmd is more helpful and professional. The md file is just code and does not 
include the figures or tables that are the results from my code.
...
**Response**: I prefer the boxplot overlapped over the jitterplot. This visual is better because the
jitterplot shows the outliers and hides the middle data points that you are not necessarily interested in.
...
**Response**: The mapping in the ggplot is synergistic with the mapping in the geom_boxplot layer.
For example the mapping in ggplot applies to all, but the individual mapping applies to that 
specific graphic.
...
ggplot(data = penguins, mapping = aes(x = bill_length_mm, color=sex)) +
  geom_histogram()
```
...
ggplot(data = penguins, mapping = aes(x = bill_length_mm)) +
  geom_histogram(color=islands)
```

**Response**: It seems like species are driving the two peaks.
...
```{r flipper_vs_mass}
ggplot(data = penguins, mapping = aes(y = flipper_length_mm, x=body_mass_g)) +
  geom_point()
```

Describe any patterns that you notice.

**Response**: There is a positive linear relationship between body mass and flipper length.
...
ggplot(data = penguins, mapping = aes(y = flipper_length_mm, x=body_mass_g, color=species, shape=species)) +
  geom_point()
```

Describe any patterns that you notice.

**Response**: Gentoo penguins seem to have a higher body mass and greater flipper length.
