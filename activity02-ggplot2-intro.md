...
**Response**: The Gentoo penguins look to be significantly different
from the other two penguins. It has a much larger flipper length. The
Chinstrap and Adilie penguins are similar, with Chinstrap tends to have
a slightly longer flipper length and Adelie has long and short flipper
outliers.
...
**Response**: You can assess individual outliers better in the
jitterplot. In the boxplot, it is less overwhelming and you can look at
the overall distribution. The boxplot seems to be more of a summary.
...
**Response**: I think the Rmd is more helpful and professional. The md
file is just code and does not include the figures or tables that are
...
**Response**: I prefer the boxplot overlapped over the jitterplot. This
visual is better because the jitterplot shows the outliers and hides the
middle data points that you are not necessarily interested in.
...
**Response**: The mapping in the ggplot is synergistic with the mapping
in the geom\_boxplot layer. For example the mapping in ggplot applies to
all, but the individual mapping applies to that specific graphic.
